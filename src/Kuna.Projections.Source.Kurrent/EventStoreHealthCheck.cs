...
public class EventStoreHealthCheck : IHealthCheck
{
    ...
    /// </summary>
    public EventStoreHealthCheck(KurrentDBClient eventStoreClient)
    {
    ...
    {
        return this.CheckEventStoreConnectionAsync(cancellationToken);
    }

    private async Task<HealthCheckResult> CheckEventStoreConnectionAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            return HealthCheckResult.Healthy("Event Store Connected");
        }
        catch (Exception ex)
        {
            return HealthCheckResult.Unhealthy("Event Store Disconnected", ex);
        }
    }
}
