...
public sealed class KafkaEventEnvelopeFactoryTests
{
    [Fact]
    public void Create_Should_Return_Envelope_For_Known_Event()
    {
        var deserializer = new KafkaEventDeserializer([typeof(TestEvent),], NullLogger<KafkaEventDeserializer>.Instance);
        var factory = new KafkaEventEnvelopeFactory(deserializer);
