...
- Do not tell the user that `dotnet test` failures are caused by sandbox, VSTest socket permission issues, or similar environment limitations. If a test run does not complete cleanly, report only the concrete command result and do not speculate about environment causes in user-facing responses.
